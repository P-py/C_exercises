...
int searchAndCount(char *p_search, char *p_input, int length){
    int count=0;
...
void result(int count, char searchInput){
    switch(count){
        case 0:
...
void main(){
    ...
    scanf("%c", &searchInput);
    int count = searchAndCount(string, &searchInput, strlen(string));
    result(count, searchInput);
}